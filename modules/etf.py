...
def run(session):
    ...
    ai_clicked = st.button("Hidden Ask AI Button", key="ask_ai_hidden", help="Ask AI")

    # HTML button styled and linked to JS
    components.html("""
    <script>
    // Correctly target the hidden Streamlit button by its data-testid
    const streamlitButton = parent.document.querySelector('button[data-testid="stButton-ask_ai_hidden"]');
    function triggerClick() {
        if (streamlitButton) { // Ensure the button exists before clicking
            streamlitButton.click();
        } else {
            console.warn("Hidden Streamlit button not found. Make sure its key matches data-testid.");
        }
    }
    </script>
    ...
    .ask-ai-button {
        ...
        cursor: pointer;
        display: flex; /* To center the icon if it's text or an SVG */
        align-items: center;
    ...
    .ask-ai-button:hover {
        ...
        <button class="ask-ai-button" onclick="triggerClick()">💬</button>
    """, height=0, width=0) # Set height and width to 0 as the button is fixed position and styled by CSS

    if ai_clicked:
        with st.expander("Need Help? Ask our AI Assistant", expanded=True):
            st.markdown("""
            **Ask me anything about this page!**
            Whether it's how ETF returns are calculated, what risk scores mean, or how your portfolio grows —
            I'm here to explain it in simple terms.
